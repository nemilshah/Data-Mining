...
<div style="padding:30px" class="main_bg">
		...
		<p style=" font-size:50px; color:rgb(164, 175, 32); margin-left:10%">TEACHER LOG IN</h1><br><br>
<form action= '' method = 'post' style="margin-left:10%" >
	<p><strong id="left"> Email-ID :</strong>	
	<input type='text' name='emailid' placeholder='Email' value=
	'<?php 
...
$us= $_POST['emailid'];
		...
		$res = $con->query("select * from teachers where email ='$us'");
		echo $us;
		if($res->num_rows==0)
		{
			...
			if(  $p==$_POST['password'])
			{
				...
				*/
				$_SESSION['teachername']=$row['name'];
				  echo '<script>window.location.assign("Tprofile.php");</script>';
